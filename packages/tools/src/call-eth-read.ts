...
import { Signer } from '@polyjuice-provider/base';
...
async function readCall() {
  ...
  const signer = new Signer(ACCOUNT_PRIVATE_KEY);
  const signature = signer.sign_with_private_key(messageToSign, '');
  ...
  const result = await godwoken.executeRawL2Transaction(polyjuiceTx);
  console.log({
    result
  });
